...
pub(super) fn parse(av: &[String]) -> Result<Args, SignError> {
    ...
    while i < av.len() {
        ...
        match av[i].as_str() {
            "--cert" => {
                a.cert = PathBuf::from(v("--cert <path>")?);
                have.0 = true;
                i += 2;
            }
            "--namespace" => {
                a.namespace = v("--namespace <s>")?.clone();
                have.1 = true;
                i += 2;
            }
            "--version" => {
                a.version = parse_version(v("--version <m.m.p>")?)?;
                have.2 = true;
                i += 2;
            }
            "--target" => {
                a.target = v("--target <triple>")?.clone();
                have.3 = true;
                i += 2;
            }
            "--elf" => {
                a.elf = PathBuf::from(v("--elf <path>")?);
                have.4 = true;
                i += 2;
            }
            "--required-caps" => {
                a.required_caps = parse_u64(v("--required-caps <hex>")?)?;
                have.5 = true;
                i += 2;
            }
            "--optional-caps" => {
                a.optional_caps = parse_u64(v("--optional-caps <hex>")?)?;
                i += 2;
            }
            "--endpoint" => {
                a.endpoints.push(parse_endpoint(v("--endpoint <k:p:n>")?)?);
                i += 2;
            }
            "--pub-seed" => {
                let (g, p) = parse_alg_path(v("--pub-seed <a=p>")?)?;
                a.pub_seeds.push((g, PathBuf::from(p)));
                i += 2;
            }
            "--out" => {
