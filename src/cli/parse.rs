...
pub fn parse_u64(s: &str) -> Result<u64, SignError> {
    ...
    match stripped {
        Some(h) => {
            u64::from_str_radix(h, 16).map_err(|_| SignError::Usage(format!("bad hex u64 `{}`", s)))
        }
        None => s
            .parse::<u64>()
            .map_err(|_| SignError::Usage(format!("bad u64 `{}`", s))),
    }
}
...
pub fn parse_version(s: &str) -> Result<(u32, u32, u32), SignError> {
    ...
    if parts.len() != 3 {
        return Err(SignError::Usage(format!(
            "version must be maj.min.patch, got `{}`",
            s
        )));
    }
    let p = |x: &str| {
        x.parse::<u32>()
            .map_err(|_| SignError::Usage(format!("bad version component `{}`", x)))
    };
