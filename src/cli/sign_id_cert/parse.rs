...
pub(super) fn parse(av: &[String]) -> Result<Args, SignError> {
    ...
    while i < av.len() {
        ...
        match av[i].as_str() {
            "--serial" => {
                a.serial = parse_u64(v("--serial <n>")?)?;
                have.0 = true;
                i += 2;
            }
            "--nonos-id" => {
                a.nonos_id = parse_hex32(v("--nonos-id <hex32>")?)?;
                have.1 = true;
                i += 2;
            }
            "--ns-glob" => {
                a.ns_globs.push(v("--ns-glob <s>")?.clone());
                i += 2;
            }
            "--caps-ceiling" => {
                a.caps_ceiling = parse_u64(v("--caps-ceiling <hex>")?)?;
                have.2 = true;
                i += 2;
            }
            "--epoch" => {
                a.epoch = parse_u64(v("--epoch <n>")?)?;
                have.3 = true;
                i += 2;
            }
            "--valid-from-ms" => {
                a.valid_from_ms = parse_u64(v("--valid-from-ms <n>")?)?;
                have.4 = true;
                i += 2;
            }
            "--valid-until-ms" => {
                a.valid_until_ms = parse_u64(v("--valid-until-ms <n>")?)?;
                have.5 = true;
                i += 2;
            }
            "--pub-key" => {
                let (g, p) = parse_alg_path(v("--pub-key <a=p>")?)?;
                a.pub_keys.push((g, PathBuf::from(p)));
                i += 2;
            }
            "--ta-seed" => {
                let (g, p) = parse_alg_path(v("--ta-seed <a=p>")?)?;
                a.ta_seeds.push((g, PathBuf::from(p)));
                i += 2;
            }
            "--metadata" => {
                a.metadata = v("--metadata <s>")?.clone();
                i += 2;
            }
            "--out" => {
                a.out = PathBuf::from(v("--out <path>")?);
                have.6 = true;
                i += 2;
            }
