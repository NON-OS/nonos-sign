...
use crate::fixtures::{
    ...
    sign_manifest_corrupt_ed, ta_policy, REQUIRED_ALGS,
};

#[test]
fn manifest_publisher_bad_sig() {
    let b = make_bundle();
    let policy_bytes = encode_policy(ta_policy(&b));
    let cert_bytes = sign_cert(cert_input(&b), &b);
    let cert_id = cert_id_of(&cert_bytes);
    let manifest_bytes = sign_manifest_corrupt_ed(manifest_input(&b, cert_id), &b);
    ...
    assert!(matches!(r, Err(SignError::VerifyPublisherBadSig(_))), "got {:?}", r);
}
