...
use crate::fixtures::{
    cert_input, encode_policy, make_bundle, sign_cert_only_ed, ta_policy, NOW_MS, REQUIRED_ALGS,
};

#[test]
fn cert_trust_anchor_policy_missing_alg() {
    let b = make_bundle();
    let policy_bytes = encode_policy(ta_policy(&b));
    let cert_bytes = sign_cert_only_ed(cert_input(&b), &b);
