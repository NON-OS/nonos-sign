...
use crate::fixtures::{
    cert_input, encode_policy, make_bundle, sign_cert, ta_policy, CERT_SERIAL, NOW_MS,
    REQUIRED_ALGS,
};

#[test]
fn cert_serial_revoked() {
    let b = make_bundle();
    let mut p = ta_policy(&b);
    p.revoked_cert_serials = vec![CERT_SERIAL];
    ...
    assert!(matches!(r, Err(SignError::VerifyCertRevoked)), "got {:?}", r);
}
